...
#        for my $Parameter (qw(SQL Max ResultFormat)) {
        for my $Parameter (qw(SQL Max ResultFormat SnippetName SaveAsSnippet DeleteSnippet)) {
# --- 
...
# ---
        if ( $Param{SaveAsSnippet} && $Param{Name} ) {
            ...
            if ( $Check{ $Param{SnippetName} } ) {
                $Method = 'SnippetUpdate';
                $Param{SnippetID} = $Check{ $Param{SnippetName} };
            }

            if ( $Param{DeleteSnippet} ) {
                $Method = 'SnippetDelete';
            }
